        ...
        <div :class="[
            ...
            <div class="md:hidden absolute inset-x-0 bottom-0 p-4 z-40 bg-gradient-to-t from-black/90 via-black/40 to-transparent pointer-events-none pb-5">
                <div class="flex flex-col gap-4 pointer-events-auto mt-8">
                    ...
                    <div class="flex gap-4 justify-end -mt-6 pr-1">
                        <button 
                            @click.stop="cart.addItem(product)"
                            class="w-10 h-10 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center active:scale-95 transition-all text-white hover:bg-white hover:text-black shadow-lg"
                        >
                            <span class="material-symbols-outlined text-[18px]">shopping_bag</span>
                        </button>
                        <a 
                            ...
                            target="_blank" 
                            @click.stop
                            class="w-10 h-10 rounded-full bg-[#25D366]/20 backdrop-blur-md flex items-center justify-center active:scale-95 transition-all hover:bg-[#25D366] text-[#25D366] hover:text-white shadow-lg"
                        >
                    ...
                    <div class="flex flex-col z-10 px-1">
                        <span class="text-[18px] font-headline font-black text-primary drop-shadow-lg mb-1">{{ formatAmount(product.price, page.props) }}</span>
                        <h3 class="text-[14px] font-headline font-medium text-white uppercase tracking-widest leading-tight drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">{{ product.name }}</h3>
                        <p class="text-[9px] mt-1.5 font-medium text-white/70 uppercase tracking-[0.3em]">{{ product.category?.name || 'Archive' }}</p>
                    </div>
                </div>
            </div>
