        ...
        <div class="flex items-center justify-between px-4 py-4 lg:px-12 w-full max-w-[1920px] mx-auto relative h-full">
                        ...
                        <div class="dark:bg-black/40 bg-white/40 backdrop-blur-3xl border dark:border-white/10 border-black/10 rounded-2xl shadow-2xl p-10 grid grid-cols-2 gap-10">
                            <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-2 gap-x-8 gap-y-4">
                                    <Link v-for="cat in ($page.props.categories || []).slice(0, 6)" :key="cat.id" 
                                          :href="route('shop', {category: cat.slug})" 
                                          class="dropdown-link" 
                                          :class="{ 'text-primary': $page.url.includes(cat.slug) }">
                                        {{ cat.name }}
                                    </Link>
                                </div>
            ...
            <div class="flex items-center gap-10 relative z-20">
                <div class="hidden lg:flex items-center gap-6 pr-6 border-r dark:border-white/10 border-black/10">
                    ...
                    <ThemeToggle />
                    <a :href="whatsappUrl" target="_blank" title="Studio WhatsApp" class="icon-btn !p-0">
